            ...
            let (version_str, op) = if let Some(version_str) = version_str.strip_suffix(".*") {
                match op {
                    ...
                    op => {
                        ...
                        //     op,
                        // ))
                        // tracing::info!("Using .* with relational operator is superfluous and deprecated and will be removed in a future version of conda. Your spec was {version_str}.*, but conda is ignoring the .* and treating it as {version_str}");
                        (version_str, op)
                    }
                }
            } else {
