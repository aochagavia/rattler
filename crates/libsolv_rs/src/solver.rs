...
impl Solver {
    ...
    pub fn solve(&mut self, jobs: SolveJobs) -> Result<Transaction, Vec<String>> {
        ...
        self.run_sat(&jobs.install, &jobs.lock)
            .map_err(|_| self.solver_problems())?;
    ...
    ) -> Result<(), Vec<SolveProblem>> {
        let level = self.install_root_solvable();

        // TODO: I'm not sure the errors generated by analyze_unsolvable will make sense if they
        // happen before resolve_deps
    ...
    fn decide_top_level_assertions(
                ...
                for &other_candidate in other_candidates {
                    if other_candidate != locked_solvable_id {
                        self.decision_tracker
                            ...
                            .expect("bug: the solvable was already decided to true");
                    }
                }
            }
        }
        ...
        // Assertions derived from top-level match specs (all candidates that do not match are
        // forbidden)
        for req in top_level_requirements {
            ...
            for &solvable_id in non_matching {
                let decided = self
                    .decision_tracker
                    .try_add_decision(Decision::new(solvable_id, false, RuleId::install_root()), 1)
                    .expect("bug: the solvable was already decided to true");
                if decided {
                    println!(
                        "{} = false",
    ...
    fn resolve_dependencies(&mut self, mut level: u32) -> Result<u32, Vec<SolveProblem>> {
        let mut i = 0;
        loop {
            if i >= self.rules.len() {
                break;
    ...
    fn analyze_unsolvable(&mut self, _rule: RuleId) -> Vec<SolveProblem> {
        // todo!()
        Vec::new()
    }
...
mod test {
    ...
    fn test_unsat_locked_and_excluded() {
        ...
        let Err(_) = solver.solve(job) else {
            panic!("expected error, but solver succeeded");
        };
    }

    #[test]
    fn test_unsat_no_candidates_for_child() {
        let pool = pool(&[("asdf", "1.2.3", vec!["C>1"]), ("C", "1.0.0", vec![])]);
        let mut solver = Solver::new(pool);
        let Err(_) = solver.solve(install(&["asdf"])) else {
            panic!("expected error, but solver succeeded");
        };
    }

    #[test]
    fn test_unsat_missing_top_level_dep() {
        let pool = pool(&[("asdf", "1.2.3", vec![])]);
        let mut solver = Solver::new(pool);
        let Err(_) = solver.solve(install(&["fghj"])) else {
            panic!("expected error, but solver succeeded");
        };
    }
    ...
    fn test_unsat_after_backtracking() {
        ...
        let mut solver = Solver::new(pool);
        let Err(_) = solver.solve(install(&["B", "C"])) else {
            panic!("expected error, but solver succeeded");
        };
    }
